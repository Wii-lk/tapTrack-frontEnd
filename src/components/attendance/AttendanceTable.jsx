...
const AttendanceTable = ({ records, loading, userType }) => {
  ...
  let headers = studentHeaders;
  if (userType === "staff") headers = staffHeaders;
  if (userType === "all") headers = mixedHeaders;

  const AttendanceCard = ({ record }) => {
    const { user, status, check_in_time, check_out_time } = record;
    const grade =
      (userType === "student" || userType === "all") && user.gradeId
        ? MOCK_GRADES.find((g) => g.id === user.gradeId)
        : null;

    // Determine the role/class subtitle
    let subTitle = "N/A";
    let subTitleLabel = "Info";

    if (user.role === "student" || userType === "student") {
      subTitle = grade ? grade.name : "N/A";
      subTitleLabel = "Class";
    } else {
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 mb-3">
        <div className="flex justify-between items-start mb-2">
          ...
          <span
            className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${getStatusChip(status)}`}
          >
            {status.replace("_", " ")}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500 border-t border-gray-50 pt-2">
          <div>
            <span className="block text-gray-400">{subTitleLabel}</span>
            {subTitle}
          </div>
          <div>
  ...
  const renderRow = (record) => {
    const { user, status, check_in_time, check_out_time } = record;
    const isStudent =
      user.role === "student" || (userType === "student" && !user.role);

    let roleOrClass = "N/A";

    if (isStudent) {
      const grade = MOCK_GRADES.find((g) => g.id === user.gradeId);
      roleOrClass = grade ? grade.name : "N/A";
    } else {
      roleOrClass = user.role || user.position || "Staff";
    }
    ...
    return (
      <tr key={record.id} className="hover:bg-gray-50">
        ...
        <td className="px-6 py-4 whitespace-nowrap">
          <span
            className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full capitalize ${getStatusChip(status)}`}
          >
            {status.replace("_", " ")}
          </span>
        </td>
