...
const AttendanceManagement = () => {
  ...
  const [userType, setUserType] = useState("staff"); // 'student' or 'staff'
  ...
  const fetchSummary = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await attendanceService.getTodayAttendanceSummary(
        userType,
        summaryFilters,
      );
  ...
  const ViewTab = ({ view, icon, label }) => {
    ...
    const base =
      "flex-1 md:flex-none px-4 py-2.5 text-sm font-medium transition-colors rounded-lg flex items-center justify-center gap-2";
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            View attendance data, live presence, and history.
          </p>
        </div>
